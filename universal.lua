--This watermark is used to delete the file if its cached, remove it to make the file persist after vape updates. so proooooo
...
run(function()
	local Freeze = vape.Categories.Blatant:CreateModule({
		Name = 'Freeze (Bare Simple)',
		Function = function(callback)
			-- Get the character's main part
			local root = lplr.Character and lplr.Character:FindFirstChild("HumanoidRootPart")
			
			if root then
				if callback then
					-- **FREEZE ACTION:** Simply anchor the part.
					root.Anchored = true
				else
					-- **UNFREEZE ACTION:** Simply unanchor the part.
					root.Anchored = false
				end
			end
		end,
		Tooltip = 'Anchors the HumanoidRootPart in place—the simplest way.'
	})
end)
	
local connections = {}
local jumpSpeed = 50 -- Default jump power (standard Roblox default)
local originalJumpPower = 50
local jumpConnection = nil

local fly = vape.Categories.Blatant:CreateModule({
	Name = 'Space Jump',
	Function = function(callback)
		...
		if callback then
			if humanoid then
				-- Store the original jump power before changing it
				originalJumpPower = humanoid.JumpPower 
				
				-- Apply the configurable speed
				humanoid.JumpPower = jumpSpeed
			end
			
			-- **USER'S LOGIC:** Connect the JumpRequest to force a jump state.
			...
			-- Handle character respawn to re-apply the jump power
			local function onCharacterAdded(char)
				local human = char:WaitForChild("Humanoid", 5)
				if human then
					human.JumpPower = jumpSpeed
				end
			end
			table.insert(connections, lplr.CharacterAdded:Connect(onCharacterAdded))

		else
			-- On module disable (Cleanup)
			
			-- Disconnect the jump handler
	...
	Tooltip = 'Holds space to continuously jump high.'
})

-- Add the configuration slider
fly:CreateSlider({
	Name = 'Jump Height/Speed',
	DefaultValue = jumpSpeed,
	Min = 5,
	Max = 200,
	Unit = 'power',
	Function = function(val)
		jumpSpeed = val
		-- Update the JumpPower immediately
		local humanoid = lplr.Character and lplr.Character:FindFirstChild("Humanoid")
		if fly.Enabled and humanoid then
			humanoid.JumpPower = jumpSpeed
		end
	end
})
...
run(function()
	...
	keycardScanTP = vape.Categories.Utility:CreateModule({
		Name = 'Keycard Scan TP (Final Invoke)',
		Function = function(callback)
			local character = lplr.Character
			
			if callback and character then
				
				-- 1. Define Remotes (Use WaitForChild for the specific path)
				local remoteContainer = workspace:WaitForChild("Remote", 5)
				local remote = remoteContainer and remoteContainer:WaitForChild("ItemHandler", 5)
				
				if not remote or not remote:IsA("RemoteFunction") then -- CRITICAL: Check for RemoteFunction
					vape:CreateNotification('Keycard Scan', 'Failed to find RemoteFunction "ItemHandler"! Cannot proceed.', 5, 'alert')
					keycardScanTP:Toggle(false)
					return
				end

				-- SAVE ORIGINAL POSITION
				local startCFrame = character:GetPrimaryPartCFrame()

				-- Function to handle the teleport and remote invoke
				local function executeKeycardInteraction(keycardModel)
					-- Ensure this only runs once
					if not keycardScanTP.Enabled then return end
					keycardScanTP.Enabled = false -- Prevent re-entry
					
					-- 2. Attempt to get the CFrame for teleport 
					...
					if centerCFrame then
						-- Perform the hop
						local tp4CFrame = CFrame.new(-924, 94, 2045)
						local onTopCFrame = centerCFrame * CFrame.new(0, size.Y / 2 + 3, 0)
						
						character:SetPrimaryPartCFrame(onTopCFrame) 
						task.wait(0.1)
						character:SetPrimaryPartCFrame(tp4CFrame)
					else
						vape:CreateNotification('Keycard Scan', 'Warning: Skipped TP (No physical parts found).', 3, 'alert')
					end
					
					-- 3. DIRECT REMOTE INTERACTION (CRITICAL FIX: Use InvokeServer)
					local success, result = pcall(function()
						return remote:InvokeServer("ITEMPICKUP") -- Use InvokeServer with the string argument
					end)

					if success and result then
						vape:CreateNotification('Keycard Scan', 'SUCCESS: Pickup confirmed by server!', 5, 'check')
					else
						vape:CreateNotification('Keycard Scan', 'FAILURE: Pickup invoked, but server response failed/rejected.', 5, 'alert')
					end
					
					-- 4. FINAL TELEPORT AND SHUTDOWN
					character:SetPrimaryPartCFrame(startCFrame)
					
					-- Disconnect all listeners and turn off the module
					for _, conn in ipairs(connections) do conn:Disconnect() end
					connections = {}
					keycardScanTP:Toggle(false)
				end

				-- **Initial Setup and Sequence Start**
				task.spawn(function()
					
					-- 1. Attach the event listener (Detection logic)
					table.insert(connections, workspace.ChildAdded:Connect(function(child)
						if child.Name == "Key card" and child:IsA("Model") then
							executeKeycardInteraction(child)
						end
					end))

					-- 2. Check if the model is already there 
					local existingKeycard = workspace:FindFirstChild("Key card")
					if existingKeycard and existingKeycard:IsA("Model") then
						executeKeycardInteraction(existingKeycard)
						return
					end

					-- 3. EXECUTE INITIAL TELEPORT SEQUENCE
					character:SetPrimaryPartCFrame(CFrame.new(814, 101, 2217))
					task.wait(0.35)

					character:SetPrimaryPartCFrame(CFrame.new(820, 101, 2217))
					task.wait(0.35)

					character:SetPrimaryPartCFrame(CFrame.new(-922, 94, 2052))
					task.wait(0.35)

					character:SetPrimaryPartCFrame(CFrame.new(-924, 94, 2045))
					task.wait(0.35)

					vape:CreateNotification('Keycard Scan', 'Initial sequence complete. Waiting for "Key card" model to appear...', 5, 'info')
		...
		Tooltip = 'Uses Event-Driven detection and INVOKES the ItemHandler remote with the string "ITEMPICKUP".'
	})
end)
